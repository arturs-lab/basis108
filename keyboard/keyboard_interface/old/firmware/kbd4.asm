...
s0:    anl     a,#7fh             ; strip 7-th bit
       jnz     s1               ; not CTRL ( code 80 )
       mov     a,#ctrl          ; set CTRL flag on
       orl     a,r4             ; get status register
       mov     r4,a
       ret
s1:    dec     a
       jnz     s2
...
sendch:mov     r6,a             ; store char for later
       ...
       anl     a,#3fh           ; and convert to ctrl code
       mov     r6,a
sc1:   orl     a,#80h           ; set clk high
...
scan:  db      '',27,'123456',  '7890-=',8,9       ; 00 - 0f
       db      'QWERTYUI',       'OP[]',13,80h,'AS'  ; 10 - 1f
       db      'DFGHJKL:',       39,'~s\ZXCV'       ; 20 - 2f
       db      'BNM,./s*',       'a kfffff'         ; 30 - 3f
